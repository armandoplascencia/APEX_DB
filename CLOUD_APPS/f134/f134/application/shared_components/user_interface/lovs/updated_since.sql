prompt --application/shared_components/user_interface/lovs/updated_since
begin
--   Manifest
--     UPDATED SINCE
--   Manifest End
wwv_flow_api.component_begin (
 p_version_yyyy_mm_dd=>'2021.04.15'
,p_release=>'21.1.7'
,p_default_workspace_id=>9008156634332785
,p_default_application_id=>134
,p_default_id_offset=>172493832712964115
,p_default_owner=>'MISO'
);
wwv_flow_api.create_list_of_values(
 p_id=>wwv_flow_api.id(3874404458966970505)
,p_lov_name=>'UPDATED SINCE'
,p_lov_query=>'.'||wwv_flow_api.id(3874404458966970505)||'.'
,p_location=>'STATIC'
);
wwv_flow_api.create_static_lov_data(
 p_id=>wwv_flow_api.id(3874404713042970533)
,p_lov_disp_sequence=>1
,p_lov_disp_value=>'Within the last 30 days'
,p_lov_return_value=>'30'
);
wwv_flow_api.create_static_lov_data(
 p_id=>wwv_flow_api.id(3874405157733970535)
,p_lov_disp_sequence=>2
,p_lov_disp_value=>'Within the last 60 days'
,p_lov_return_value=>'60'
);
wwv_flow_api.create_static_lov_data(
 p_id=>wwv_flow_api.id(3874405551605970535)
,p_lov_disp_sequence=>3
,p_lov_disp_value=>'Within the last 90 days'
,p_lov_return_value=>'90'
);
wwv_flow_api.component_end;
end;
/
