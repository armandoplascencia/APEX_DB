prompt --application/shared_components/user_interface/lovs/email_target_all_or_user
begin
--   Manifest
--     EMAIL TARGET ALL OR USER
--   Manifest End
wwv_flow_api.component_begin (
 p_version_yyyy_mm_dd=>'2021.04.15'
,p_release=>'21.1.7'
,p_default_workspace_id=>9008156634332785
,p_default_application_id=>134
,p_default_id_offset=>172493832712964115
,p_default_owner=>'MISO'
);
wwv_flow_api.create_list_of_values(
 p_id=>wwv_flow_api.id(11985717958624851140)
,p_lov_name=>'EMAIL TARGET ALL OR USER'
,p_lov_query=>'.'||wwv_flow_api.id(11985717958624851140)||'.'
,p_location=>'STATIC'
);
wwv_flow_api.create_static_lov_data(
 p_id=>wwv_flow_api.id(11985718170596851146)
,p_lov_disp_sequence=>10
,p_lov_disp_value=>'All Users'
,p_lov_return_value=>'ALL'
);
wwv_flow_api.create_static_lov_data(
 p_id=>wwv_flow_api.id(11985718555001851149)
,p_lov_disp_sequence=>20
,p_lov_disp_value=>'Single User'
,p_lov_return_value=>'USER'
);
wwv_flow_api.component_end;
end;
/
