...
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
CREATE OR REPLACE PROCEDURE get_schema_md4 (v_schema_extract in varchar2, v_options_false in varchar2)
authid current_user
 as
...
holdsizeof      number;
v_token1        my_metadata.token1%type;
v_token2        my_metadata.token2%type;
v_token3        my_metadata.token3%type;
v_token4        my_metadata.token4%type;
v_token5        my_metadata.token5%type;
v_token6        my_metadata.token6%type;
v_token7        my_metadata.token7%type;
v_token8        my_metadata.token8%type;
v_token9        my_metadata.token9%type;
v_token10       my_metadata.token10%type;
...
v_code          number := 0;          ---->:= SQLCODE;
v_errm          varchar2(4000) := ''; ---->:= SUBSTR(SQLERRM, 1 , 64);
v_handleof_loc   varchar2(400)  := 'my_dbms_output';   ---->'procedure print_clob';
 PRAGMA AUTONOMOUS_TRANSACTION;
BEGIN
...
then
 ...
 begin
  insert into make_perm_my_dbms_output_tbl (pk_id, text_out, group_name) (select * from my_dbms_output_tbl);
  commit;
  delete from  my_dbms_output_tbl;
  commit;
 ...
 exception when others
  then null;
   ...
   LOOP
      ...
      insert into my_dbms_output_tbl (text_out, group_name) values (v_hold_cur, v_groupname);
      l_str := SUBSTR(l_str,l_instr+1);
   END LOOP;
 commit;
exception when others then
         ...
         v_errm := SUBSTR(SQLERRM, 1 , 4000);
         ---DBMS_OUTPUT.PUT_LINE('WHO=>'||v_handleof_loc||' Error_code=>' || v_code || ' Error_msg=>' || v_errm);
         insert into my_dbms_output_tbl (text_out, group_name) values ('WHO=>'||v_handleof_loc||' Error_code=>' || v_code || ' Error_msg=>' || v_errm , v_groupname);
END;
--------------------------------------------------------------------------------
...
as
begin
v_handleof_loc := 'procedure delete_from_my_metadata';
     delete from my_metadata where otype in ('REMOVE','TRASH','PLSQL OPT','STATS') and
...
as
begin
v_handleof_loc := 'PROCEDURE CLEANUP_MY_METADATA';
update my_metadata set mdsize = dbms_lob.getlength(md);
update my_metadata set otype='REMOVE'         where mdsize = 5;
update my_metadata set otype='GTTABLE'        where  first_200 like '%CREATE%GLOBAL%TEMPORARY%TABLE%';
update my_metadata set otype='CR8UNIQUEINDEX' where  first_200 like '%CREATE%UNIQUE%INDEX%';
update my_metadata set otype='ALTUNIQUEINDEX' where  first_200 like '%ALTER%TABLE% ADD UNIQUE %';
update my_metadata set otype='ALTUNIQUEINDEX' where  first_200 like '%ALTER%TABLE% ADD CONSTRAINT% UNIQUE %';
update my_metadata set otype='ALTCHECKCONST' where  first_200 like '%ALTER%TABLE% ADD CONSTRAINT% CHECK %';

update my_metadata set otype='INDEX'       where  first_200 like '%CREATE%INDEX%';
update my_metadata set otype='TABLE'       where  first_200 like '%CREATE%TABLE%';
update my_metadata set otype='TRASH'       where  first_200 like '%sys.dbms_logrep_imp.instantiate_schema(schema_name=>SYS_CONTEXT%';
update my_metadata set otype='SEQUENCE'    where  first_200 like '%CREATE%SEQUENCE%';
update my_metadata set otype='SYNONYM'     where  first_200 like '%CREATE%OR%REPLACE%SYNONYM%';
update my_metadata set otype='LINK'        where  first_200 like '%CREATE%DATABASE%LINK%';
update my_metadata set otype='GRANT SIUD'  where  first_200 like '%GRANT %SELECT%ON%' or
...
update my_metadata set otype='GRANT INDEX ON'  where  first_200 like '%GRANT INDEX ON%';
update my_metadata set otype='GRANT REFERENCES ON'  where  first_200 like '%GRANT REFERENCES ON%';
update my_metadata set otype='GRANT ALTER ON'  where  first_200 like '%GRANT ALTER ON%';

update my_metadata set otype='CREATE_PK'   where  first_200 like '%ALTER%TABLE%'    and
                                                           first_200 like '%ADD%CONSTRAINT%' and
                                                           first_200 like '%PRIMARY KEY%';
update my_metadata set otype='CREATE_FK'   where  first_200 like '%ALTER%TABLE%'    and
                                                           first_200 like '%FOREIGN%KEY%';

update my_metadata set otype='CREATE_MVLOG' where first_200 like '%CREATE MATERIALIZED VIEW LOG';

update my_metadata set otype='CREATE_MV'    where first_200 like '%CREATE MATERIALIZED VIEW%' and
...
update my_metadata set otype='CREATE CHECK' where first_200 like '%ALTER TABLE%'    and
                                                           first_200 like '%ADD CHECK%';

update my_metadata set otype='COMMENT'  where     first_200 like '%COMMENT ON%' or
                                                           first_200 like '%COMMENT ON TABLE%' or
                                                           first_200 like '%MMENT ON MATERIALIZED VIEW%';


update my_metadata set otype='FUNCTION'  where    first_200 like '%CREATE OR REPLACE FUNCTION%';
update my_metadata set otype='PROCEDURE' where    first_200 like '%CREATE OR REPLACE PROCEDURE%';
update my_metadata set otype='PACKAGE'   where    first_200 like '%CREATE OR REPLACE PACKAGE%';
update my_metadata set otype='TYPE'      where    first_200 like '%CREATE OR REPLACE TYPE%';
update my_metadata set otype='PLSQL OPT' where    first_200 like '%ALTER PROCEDURE%'    and
                                                           first_200 like '%COMPILE%';
update my_metadata set otype='PLSQL OPT' where    first_200 like '%ALTER FUNCTION%'    and
                                                           first_200 like '%COMPILE%';
update my_metadata set otype='PLSQL OPT' where    first_200 like '%ALTER PACKAGE%'    and
                                                           first_200 like '%COMPILE%';
update my_metadata set otype='PLSQL OPT' where    first_200 like '%ALTER TYPE%'    and
                                                           first_200 like '%COMPILE%';

update my_metadata set otype='VIEW' where         first_200 like '%CREATE OR REPLACE FORCE VIEW%';
update my_metadata set otype='TRIGGER' where      first_200 like '%CREATE OR REPLACE TRIGGER%';

update my_metadata set otype='DBMS_JOBS' where dbms_lob.instr(md,'BEGIN SYS.DBMS_JOB.SUBMIT',1,1)> 0;
update my_metadata set otype='STATS'     where dbms_lob.instr(md,'DBMS_STATS.IMPORT_INDEX_STATS',1,1)> 0 or
...
LOOP
   begin /* wrap loop*/
      ...
      exception when others then
       ...
       INSERT INTO my_metadata ( original_md, md, owner ) VALUES
         (doc,
          doc,
          v_schema_extract ) returning  my_metadata_id into pk_i;

       update my_metadata set first_200 = v_first_200,
                               token1  = v_token1 ,
                               token2  = v_token2 ,
                               token3  = v_token3 ,
                               token4  = v_token4 ,
...
my_dbms_output('GET_SCHEMA_MD4','META start commenting storage for user '||v_schema_extract||' '||to_char(sysdate,'yyyy/mm/dd hh24:mi:ss'));
 for ij in (select my_metadata_id, md , otype from my_metadata where --->otype like  '%TABLE%' and
...
update  my_metadata  set done_flag='Y' where owner = v_schema_extract;
update  my_metadata set mdsize = dbms_lob.getlength(md);

update my_metadata set oname = replace(token6,'"','')                         where otype in ('ALTCHECKCONST','ALTUNIQUEINDEX') and token5 = 'CONSTRAINT';
update my_metadata set oname = 'CHECK_SQL'                                    where otype in ('CREATE CHECK')                   and token5 = 'CHECK';
update my_metadata set oname = replace(token6,'"','')                         where otype in ('CREATE_FK')                      and token5 = 'CONSTRAINT';
update my_metadata set oname = replace(token6,'"','')                         where otype in ('CREATE_PK')                      and token5 = 'CONSTRAINT';
update my_metadata set oname = replace(replace(token5,'"',''),OWNER||'.','')  where otype in ('FUNCTION')                       and token4 = 'FUNCTION';
update my_metadata set oname = replace(replace(token4,'"',''),OWNER||'.','')  where otype in ('GRANT ALTER ON','GRANT SIUD')    and token1 = 'GRANT';
update my_metadata set oname = replace(replace(token3,'"',''),OWNER||'.','') ,
                  table_name = replace(replace(token5,'"',''),OWNER||'.','')  where otype in ('INDEX')                          and token1 = 'CREATE' and token2='INDEX';
update my_metadata set oname = replace(replace(token4,'"',''),OWNER||'.','') ,
                  table_name = replace(replace(token6,'"',''),OWNER||'.','')  where otype in ('INDEX')                          and token1 = 'CREATE' and token2='BITMAP';
update my_metadata set oname = replace(replace(token5,'"',''),OWNER||'.','')  where otype in ('PACKAGE');
update my_metadata set oname = replace(replace(token6,'"',''),OWNER||'.','')  where otype in ('PACKAGE_BODY');
update my_metadata set oname = replace(replace(token5,'"',''),OWNER||'.','')  where otype in ('PROCEDURE');
update my_metadata set oname = replace(replace(token3,'"',''),OWNER||'.','')  where otype in ('SEQUENCE');
update my_metadata set oname = replace(replace(token5,'"',''),OWNER||'.','') ,
                  table_name = substr( replace(replace(token7,'"',''),OWNER||'.','') ,instr(replace(replace(token7,'"',''),OWNER||'.',''),'.')+1,999)  where otype in ('SYNONYM');
update my_metadata set oname = replace(replace(token3,'"',''),OWNER||'.','') ,
                       OTYPE = 'EXT_TABLE',
                  table_name = substr( replace(replace(token3,'"',''),OWNER||'.','') ,instr(replace(replace(token7,'"',''),OWNER||'.',''),'.')+1,999)
       where otype in ('TABLE') and dbms_lob.instr(md,'ORGANIZATION EXTERNAL',1,1) > 0;
update my_metadata set oname = replace(replace(token3,'"',''),OWNER||'.','') ,
                  table_name = replace(replace(token3,'"',''),OWNER||'.','')
       where otype in ('TABLE') and dbms_lob.instr(md,'ORGANIZATION EXTERNAL',1,1) = 0;
update my_metadata set oname = replace(replace(token5,'"',''),OWNER||'.','') ,
                 table_name =   substr( replace(replace(token9,'"',''),OWNER||'.','') ,instr(replace(replace(token9,'"',''),OWNER||'.',''),'.')+1,999) where otype in ('TRIGGER');
update my_metadata set oname = replace(replace(token5,'"',''),OWNER||'.','') ,
                 table_name =   replace(substr( replace(replace(token9,'"',''),OWNER||'.','') ,instr(replace(replace(token9,'"',''),OWNER||'.',''),'.')+1,999),';','') where otype in ('TYPE') and upper(token7)='TABLE';
update my_metadata set oname = replace(replace(token5,'"',''),OWNER||'.','') ,
                 table_name =   replace(substr( replace(replace(token9,'"',''),OWNER||'.','') ,instr(replace(replace(token9,'"',''),OWNER||'.',''),'.')+1,999),';','') where otype in ('TYPE') and upper(token7) <> 'TABLE';
update my_metadata set oname = replace(replace(token4,'"',''),OWNER||'.','') ,
                  table_name = replace(replace(token6,'"',''),OWNER||'.','')  where otype in ('UINDEX');
update my_metadata set oname = replace(replace(token6,'"',''),OWNER||'.','')  where otype in ('VIEW') and TOKEN4 = 'FORCE';
...
REM declare
REM   2          no_grant exception;
REM   3          pragma exception_init( no_grant, -31608 );
REM   4  begin
REM   5          :x := dbms_metadata.get_granted_ddl( 'SYSTEM_GRANT', 'A' );
REM   6  exception
REM   7          when no_grant then :x := '-- no system grants';
REM   8  end;
REM   9  /
...
CREATE OR REPLACE 
PROCEDURE push_profiles_verify_props2 (v_schema_name in varchar2, v_output in out varchar2)
    AUTHID CURRENT_USER
as
...
v_handleof_loc  varchar2(400) := 'push_profiles_verify_props';
v_code          number := 0;          ---->:= SQLCODE;
v_errm          varchar2(4000) := ''; ---->:= SUBSTR(SQLERRM, 1 , 64);
-----------
     no_grant exception;
...
begin
 ...
 insert into my_metadata_user (created_date, instance_name   , MDSIZE   , OWNER  , ONAME       ,  original_md ) values
...
begin
 ...
 --select instance_name into v_instance_name from v$instance;
   ...
   ---------
   begin
   select count(*) into rowscount from my_metadata_user where owner = v_schema_name and instance_name = v_instance_name;
  dbms_output.put_line('Rows found that will be deleted for owner='||v_schema_name||' db_name='||v_instance_name||'=>'||to_char(rowscount));
   ...
   ---------
   begin
    delete from my_metadata_user where owner = ltrim(rtrim(upper(v_schema_name))) and instance_name =  ltrim(rtrim(upper(v_instance_name)));
...
where  tq.username = v_schema_name and rownum < 2
 ...
 create or replace view MISO.drive_rename_app1
  as
   with abc as (select distinct owner, otype  from MISO.my_metadata
    ...
    fromusr as (select 'SMSDBO.'     userofnow from dual),
      tousr as (select 'SELLOUTDBO.'   toofnow from dual)
 select * from
 (SELECT rowid as rowidof, a.md, a.mdsize,
...
select 'exec MISO.get_schema_md4('||chr(39)||username||chr(39)||','||chr(39)||'~SEGMENT_ATTRIBUTES~~STORAGE~~TABLESPACE~'||chr(39)||');' from dba_users 
 where ---username in     (select distinct owner from dba_objects) and
          username not in (select username from  MISO.re_upg_known_oracle_acts )  
...
--exec MISO.get_schema_md4('APEX_050000','~SEGMENT_ATTRIBUTES~~STORAGE~~TABLESPACE~');
exec MISO.get_schema_md4('MISO','~SEGMENT_ATTRIBUTES~~STORAGE~~TABLESPACE~');

HERE


exec MISO.get_schema_md4('TELLYROVIDBO','~SEGMENT_ATTRIBUTES~~STORAGE~~TABLESPACE~');
exec MISO.get_schema_md4('TELLYODSDBO','~SEGMENT_ATTRIBUTES~~STORAGE~~TABLESPACE~');
exec MISO.get_schema_md4('TELLYOGGDBO','~SEGMENT_ATTRIBUTES~~STORAGE~~TABLESPACE~');
exec MISO.get_schema_md4('TELLYSQLDBO','~SEGMENT_ATTRIBUTES~~STORAGE~~TABLESPACE~');
--exec MISO.get_schema_md4('TEST','~SEGMENT_ATTRIBUTES~~STORAGE~~TABLESPACE~');




--SQL> create table my_metadata_backup as select * from my_metadata;
--Table created.
create or replace procedure run_get_schema_md4_telly_app
 as 
 voutput varchar2(4000) := '';
begin
 insert into  my_metadata_backup (select * from my_metadata);
 voutput := voutput||' insert successful'||chr(10);
 commit;
 voutput := voutput||' commit successful'||chr(10);
 delete from my_metadata;
 voutput := voutput||' delete successful'||chr(10);
 commit;
 voutput := voutput||' commit successful'||chr(10);
 MISO.get_schema_md4('TELLYROVIDBO','~SEGMENT_ATTRIBUTES~~STORAGE~~TABLESPACE~')
          voutput := voutput||' TELLYROVIDBO successful'||chr(10);
 MISO.get_schema_md4('TELLYODSDBO','~SEGMENT_ATTRIBUTES~~STORAGE~~TABLESPACE~');
          voutput := voutput||' TELLYODSDBO successful'||chr(10);
 MISO.get_schema_md4('TELLYOGGDBO','~SEGMENT_ATTRIBUTES~~STORAGE~~TABLESPACE~');
          voutput := voutput||' TELLYOGGDBO successful'||chr(10);
 MISO.get_schema_md4('TELLYSQLDBO','~SEGMENT_ATTRIBUTES~~STORAGE~~TABLESPACE~');
          voutput := voutput||' TELLYSQLDBO successful'||chr(10);
exception when others then 
  raise;
end;
/
...
=========
   with abc as (SELECT a.secs_per_exec, a.inst_id, a.user_id, a.session_id,
       a.session_serial#, a.username, a.module, a.executions,
       a.kept_versions, a.cpu_seconds, a.elapsed_seconds,
       a.ttl_wait_time, a.sql_text
  FROM MISO.aap_top_sql_last_hour a where a.username like '%APEX%')
  select count(*), username, module, session_id, sum(cpu_seconds), sum(ttl_wait_time) from abc group by  username, module, session_id
  /
   with  freeof  as (  select sum(bytes/1024000) megs , tablespace_name from dba_free_Space  group by tablespace_name order by tablespace_name),
         totalof as (  select sum(bytes/1024000) megs , tablespace_name from dba_data_files  group by tablespace_name order by tablespace_name ),
           subtl as ( select (totalof.megs - freeof.megs) usedof,  freeof.megs, freeof.tablespace_name
                          from freeof, totalof
                       where freeof.tablespace_name = totalof.tablespace_name)
  select   usedof, megs, round(usedof/megs) , tablespace_name
   from subtl
 /
