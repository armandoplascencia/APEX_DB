...
#! remove know files
for ijk in `ls -1  | grep -v CURL_OUTPUT_ | grep -v tmp_files_fnd_ | grep -v new_files_to_add_to_master |  grep -v find_files_post.ksh | grep -v POS_15_MIN_FILES_PROCESSED.txt | grep -v "backup/" | grep -v send_to_curl | grep -v grep`
do
 #!
 if [[ "${ijk}" == " " ]]
 then
